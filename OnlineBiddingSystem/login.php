...
if (isset($_POST['next2'])) {
	...
	// Prepare and bind the registration data
	$stmt = $con->prepare("INSERT INTO member (lastname, firstname, gender, userid, password, email, contactno, birthdate, address, verification, memberimg) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'no', 'default.jpg')");
	$stmt->bind_param("sssssssss", $lname, $fname, $gender, $userid, $password, $email, $contactno, $birthdate, $cusaddress);
	$stmt->execute();
	$stmt->close();

	// Retrieve the inserted member ID
	$id = $con->insert_id;

	// Insert the security question and answer
	$stmt = $con->prepare("INSERT INTO secretquestions (memberid, question, answer) VALUES (?, ?, ?)");
	$stmt->bind_param("iss", $id, $secques, $secanswer);
	$stmt->execute();
	$stmt->close();
...
if (isset($_POST['login'])) {
	if (isset($_POST['user']) && isset($_POST['pass'])) {
		...
		$stmt = $con->prepare("SELECT * FROM member WHERE userid = ? AND password = ?");
