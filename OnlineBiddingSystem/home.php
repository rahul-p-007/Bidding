...
require("htmls.php");
$query = mysqli_query($con, "SELECT * FROM products WHERE status = 0") or die(mysqli_error($con));
while ($row = mysqli_fetch_array($query)) {
  $datenow = date("Y-m-d");
  $duedate = $row['duedate'];
  $prodid = $row['productid'];
  if ($datenow >= $duedate) {
    mysqli_query($con, "UPDATE products SET status = 1 WHERE productid = '$prodid'") or die(mysqli_error($con));
  }
}
headhtml();
?>
