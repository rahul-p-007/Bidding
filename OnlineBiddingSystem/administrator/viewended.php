	...
	<div id="container">
		...
		<div id="bodycon">
			<table id="demoTable" style="border: 1px solid #ccc;" cellspacing="0" width="700">
				...
				<tbody>
					<?php
					$bids_stat = mysqli_query($con, "SELECT * FROM products WHERE status = 0") or die(mysqli_error($con));
					while ($stat = mysqli_fetch_array($bids_stat)) {
						$prodid = $stat['productid'];
						$numbidderq = mysqli_query($con, "SELECT COUNT(*) FROM bidreport WHERE productid = '$prodid'") or die(mysqli_error($con));
